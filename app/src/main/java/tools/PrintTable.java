...
public class PrintTable<E> {
    ...
    public static <E> PrintTable<E> of(Collection<E> content) {
        PrintTable<E> rsl = new PrintTable<E>();
    ...
    public <CVT> PrintTable<E> column(String name, Function<E, CVT> getValue,
        ...
        ColumnOptions<E> options = new ColumnOptions<E>();
    ...
    private String buildTable() {
        ...
        String splitLine = "+" + repeatChar('-', header.length() - 2) + "+";
    ...
    private String buildHeader() {
        ...
        for (Column<E, ?> column : columns) {
            ...
            String minWidth = Optional.ofNullable(column.options).map(ColumnOptions::getMinWidth).orElse(ColumnOptions.AUTO_FIT);
            int alignSpaceNum = (ColumnOptions.AUTO_FIT.equals(minWidth))
                    ? column.maxCellWidth - column.name.length()
                    : Integer.parseInt(minWidth) - column.name.length();
            
            String cellContent = column.name;
            AlignSide alignSide = Optional.ofNullable(column.options).map(ColumnOptions::getAlignSide).orElse(AlignSide.LEFT);
            if (alignSpaceNum > 0) {
    ...
    private String buildLine(int lineIndex) {
        StringBuilder rsl = new StringBuilder();
        for (Column<E, ?> column : columns) {
            String cellContent = column.content[lineIndex];
            rsl.append("| ");
            
            String minWidth = Optional.ofNullable(column.options).map(ColumnOptions::getMinWidth).orElse(ColumnOptions.AUTO_FIT);
            int cellContentLength = (column.isCellColorized[lineIndex])
    ...
    private String repeatChar(char c, int times) {
        ...
        for (int i = 0; i < Math.max(0, times); i++) sb.append(str);
        return sb.toString();
    }
...
class Column<E, CVT> {
    ...
    public void prepareAllCellsContent(Collection<E> tableContent) {
        ...
        int contentIndex = 0;
        for (E each : tableContent) {
            String cellValue = cellValueToString(getValue.apply(each), formatter);
            int cellValueWidth = cellValue.length();
            
            if (options != null) {
                for (CellColour<E> cc : options.getCellColours()) {
...
class DefaultFormatterBuilder {
    ...
    public static Function<LocalDateTime, String> localDateTime(String pattern) {
        return numberObj -> String.valueOf((Object) null);
